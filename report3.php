    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" style="background-image:url('images/1.jpg') ;background-repeat: no-repeat; background-position: center;background-size: cover;">
            <center>  <b> <h1  style="color:green;font-weight: 10000; text-shadow: 2px 2px green;">ആകെ റിപ്പോര്‍ട്ട്</h1></b></center>
...
<?php
            ...
            $sql="SELECT sum(amount) as su FROM others WHERE status=1 ";
                $res=mysqli_query($con,$sql);
                $row1=  mysqli_fetch_array($res);
               
                 $sql="SELECT sum(amount) as su FROM others WHERE status=0 ";
                $res=mysqli_query($con,$sql);
                $row2=  mysqli_fetch_array($res);
                 $sql="SELECT name FROM month WHERE id='$month'";
                $res=mysqli_query($con,$sql);
                $row3=  mysqli_fetch_array($res);
                   $sql="SELECT sum(amount)as suma FROM interest WHERE  status=1";
                $res=mysqli_query($con,$sql);
                $row4=  mysqli_fetch_array($res);
                  $sql="SELECT sum(p_amount)as suma FROM loan  ";
                $res=mysqli_query($con,$sql);
                $row5=  mysqli_fetch_array($res);
                    $sql="SELECT sum(amount)as suma FROM week_register WHERE 1  ";
                $res=mysqli_query($con,$sql);
                $row6=  mysqli_fetch_array($res);
              $sql="SELECT distinct member_id FROM week_register WHERE amount<1 ";
                $res=mysqli_query($con,$sql);
                 $sql="SELECT distinct member_id FROM interest WHERE status is NULL OR status=0";
